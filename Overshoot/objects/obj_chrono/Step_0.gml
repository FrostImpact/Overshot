if global.paused == false {
	event_inherited()

	var gm = obj_game_manager
	var g_spd = gm.game_speed
	var px = obj_player.x
	var py = obj_player.y

	if (place_meeting(x, y, obj_wall)) {
	    for (var i = 1; i < 20; i++) {
	        if (!place_meeting(x + i, y, obj_wall)) { x += i; break; }
	        if (!place_meeting(x - i, y, obj_wall)) { x -= i; break; }
	        if (!place_meeting(x, y + i, obj_wall)) { y += i; break; }
	        if (!place_meeting(x, y - i, obj_wall)) { y -= i; break; }
	    }
	}

	if (state != 4 && state != 7 && state != 9) {
	    visual_angle = point_direction(x, y, px, py)
	}

	shatter_cooldown = max(0, shatter_cooldown - g_spd)
	special_cooldown = max(0, special_cooldown - g_spd)

	if (obj_player.aim_check && shatter_cooldown <= 0) {
	    aim_punish_timer++;
    
	    if (aim_punish_timer > 90) {
	        state = 5;
	        timer = 60;
	        aim_punish_timer = 0;
	        shatter_cooldown = 600;
        
	        obj_player.aim_check = false;
	        gm.game_speed = 1;
	        gm.boss_slow = false; 
        
	        textbox_say(["You think you can slow down time against me?", "ME??!"], c_lime, true, 30);
       
	        instance_create_layer(0, 0, layer, obj_shatter);
	        part_particles_create(global.p_sys, x, y, global.p_spark, 25);
	    } else if (aim_punish_timer > 60) {
	        part_particles_create(global.p_sys, x + random_range(-10, 10), y + random_range(-10, 10), global.p_spark, 1);
	    }
	} else {
	    aim_punish_timer = max(0, aim_punish_timer - 0.5);
	}

	if (state != 6) {
	    if (enemy_hp <= 0) {
	        state = 6;
	        gm.boss_slow = false; 
	        textbox_say(["Impossible...", "My time... has run out..."], c_lime, true, 30);
	    } else if (phase == 1 && enemy_hp <= enemy_max_hp * 0.2) {
	        phase = 2;
	        target_hp = enemy_hp + (enemy_max_hp * 0.1);
	        state = 4;
	        timer = 120;
	        gm.boss_slow = false;
	        textbox_say(["You really thought it would be that easy?", "Time is a construct...", "And I can always just REWIND!", "Let's speed things up!"], c_lime, true, 30);
	    }
	}

	if (state == 1) {
	    part_particles_create(global.p_sys, x + random_range(-15, 15), y + random_range(-15, 15), global.p_spark, 1);
	} else if (state == 3 || state == 9) {
	    part_type_orientation(global.p_trail, visual_angle, visual_angle, 0, 0, false);
	    part_type_scale(global.p_trail, visual_xscale, visual_yscale);
	    part_particles_create(global.p_sys, x, y, global.p_trail, 1);
	}

	var base_timer = (phase == 1) ? 25 : 18;
	var spec_cooldown_set = (phase == 1) ? 300 : 200;

	switch (state) {
	    case 0:
	        timer -= g_spd;
	        if (timer <= 0) {
	            if (special_cooldown <= 0 && choose(0, 1)) {
	                var selected_attack = (phase == 2) ? choose(1, 2, 3) : choose(1, 3);
	                special_cooldown = spec_cooldown_set;
                
	                if (selected_attack == 1) {
	                    state = 2;
	                    timer = (phase == 1) ? 20 : 12;
                    
	                    var dir = visual_angle + random_range(-30, 30);
	                    targ_x = x;
	                    targ_y = y;
                                        
	                    while (!place_meeting(targ_x, targ_y, obj_wall) && clamp(targ_x, 1, room_width - 1) == targ_x && clamp(targ_y, 1, room_height - 1) == targ_y) {
	                        targ_x += lengthdir_x(16, dir);
	                        targ_y += lengthdir_y(16, dir);
	                    }
                   
	                    targ_x -= lengthdir_x(24, dir);
	                    targ_y -= lengthdir_y(24, dir);
	                } else if (selected_attack == 2) {
	                    state = 7;
	                    timer = 25; 
	                } else if (selected_attack == 3) {
	                    // SETUP: Wall-to-Wall Path
	                    state = 8;
	                    path_points = []; 
	                    sim_x = x;
	                    sim_y = y;
	                    points_generated = 0;
	                    point_timer = 0; // Triggers first point immediately
	                    timer = (phase == 1) ? 30 : 20; // Final delay before dash begins
	                }
	            } else {
	                state = 1;
	                timer = 15;
	                targ_x = lengthdir_x(18, visual_angle);
	                targ_y = lengthdir_y(18, visual_angle);
	            }
	        }
	        break;

	    case 1:
	        var nx = x + targ_x * g_spd;
	        var ny = y + targ_y * g_spd;
        
	        if (!place_meeting(nx, y, obj_wall)) x = nx;
	        if (!place_meeting(x, ny, obj_wall)) y = ny;
        
	        timer -= g_spd;
	        if (timer <= 0) {
	            var knife = instance_create_layer(x, y, layer, obj_knife);
	            knife.direction = visual_angle; 
	            knife.speed = 14 * g_spd;
	            knife.image_angle = knife.direction;
            
	            x -= lengthdir_x(8, knife.direction);
	            y -= lengthdir_y(8, knife.direction);
            
	            state = 0;
	            timer = base_timer;
	        }
	        break;

	    case 2:
	        timer -= g_spd;
	        x += random_range(-2, 2) * g_spd;
	        y += random_range(-2, 2) * g_spd;
        
	        if (timer <= 0) {
	            state = 3;
	            timer = (phase == 1) ? 12 : 8;
			
	            gm.boss_slow = true;
	            current_dash_speed = 0;
			
	            part_particles_create(global.p_sys, x, y, global.p_spark, 15);
	        }
	        break;

	    case 3:
	        var dist = point_distance(x, y, targ_x, targ_y);
	        var dir = point_direction(x, y, targ_x, targ_y);
        
	        current_dash_speed = (dist > 150) ? min(current_dash_speed + (3.5 * g_spd), 45 * g_spd) : max(current_dash_speed * 0.75, 4 * g_spd);
        
	        var spd = min(current_dash_speed, dist);
	        var nx = x + lengthdir_x(spd, dir);
	        var ny = y + lengthdir_y(spd, dir);
        
	        var hit_wall = false;
	        if (!place_meeting(nx, y, obj_wall)) x = nx; else hit_wall = true;
	        if (!place_meeting(x, ny, obj_wall)) y = ny; else hit_wall = true;
        
	        timer -= g_spd;
	        if (timer <= 0) {
	            var knife = instance_create_layer(x, y, layer, obj_knife);
	            knife.direction = visual_angle;
	            knife.speed = 16 * g_spd;
	            knife.image_angle = knife.direction;
	            timer = (phase == 1) ? 6 : 4;
	        }
        
	        if (dist < 5 || hit_wall) {
	            gm.boss_slow = false;
	            state = 0;
	            timer = base_timer;
	            part_particles_create(global.p_sys, x, y, global.p_spark, 10);
	        }
	        break;

	    case 4:
	        timer -= g_spd;
	        visual_angle += (45 - (timer * 0.35)) * g_spd;
        
	        var s_dist = 30 + (timer * 0.8);
	        var s_dir = random(360);
	        part_particles_create(global.p_sys, x + lengthdir_x(s_dist, s_dir), y + lengthdir_y(s_dist, s_dir), global.p_vortex, 1);
        
	        if (enemy_hp < target_hp) enemy_hp += (target_hp - enemy_hp) * 0.05 * g_spd;
        
	        if (timer <= 0) {
	            visual_angle = 0;
	            enemy_hp = target_hp;
	            state = 0;
	            timer = base_timer;
	            part_particles_create(global.p_sys, x, y, global.p_spark, 40);
	        }
	        break;

	    case 5:
	        if (--timer <= 0) {
	            state = 0;
	            timer = base_timer;
	        }
	        break;

	    case 6: 
	        current_dash_speed = 0;
	        if (!instance_exists(obj_text_box)) instance_destroy();
	        break;
        
	    case 7:
	        timer -= g_spd;
	        visual_angle += 25 * g_spd; 
        
	        if (timer <= 0) {
	            for (var i = 0; i < 360; i += 45) {
	                var knife = instance_create_layer(x, y, layer, obj_knife);
	                knife.direction = i;
	                knife.speed = 10 * g_spd;
	                knife.image_angle = i;
	            }
	            state = 0;
	            timer = base_timer;
	            part_particles_create(global.p_sys, x, y, global.p_spark, 30);
	        }
	        break;

	    case 8:
	        x += random_range(-2, 2) * g_spd;
	        y += random_range(-2, 2) * g_spd;

	        if (points_generated < 5) {
	            point_timer -= g_spd;
	            if (point_timer <= 0) {

	                var aim_dir = point_direction(sim_x, sim_y, px, py) + random_range(-10, 10);
	                var ray_x = sim_x;
	                var ray_y = sim_y;
                    
	                while (!place_meeting(ray_x, ray_y, obj_wall) && clamp(ray_x, 1, room_width - 1) == ray_x && clamp(ray_y, 1, room_height - 1) == ray_y) {
	                    ray_x += lengthdir_x(16, aim_dir);
	                    ray_y += lengthdir_y(16, aim_dir);
	                }
	                ray_x -= lengthdir_x(24, aim_dir);
	                ray_y -= lengthdir_y(24, aim_dir);

	                array_push(path_points, { px: ray_x, py: ray_y });

	                sim_x = ray_x;
	                sim_y = ray_y;
	                points_generated++;
	                point_timer = (phase == 1) ? 12 : 8; 
                    
	                part_particles_create(global.p_sys, ray_x, ray_y, global.p_spark, 5);
	            }
	        } else {

	            timer -= g_spd;
	            if (timer <= 0) {
	                state = 9;
	                current_path_index = 0;
	                gm.boss_slow = true;
	                current_dash_speed = 0;
	                part_particles_create(global.p_sys, x, y, global.p_spark, 20);
	            }
	        }
	        break;

	    case 9: 
	        if (array_length(path_points) > 0 && current_path_index < array_length(path_points)) {
	            var target_p = path_points[current_path_index];
	            var dist = point_distance(x, y, target_p.px, target_p.py);
	            var dir = point_direction(x, y, target_p.px, target_p.py);

	            visual_angle = dir;
	            current_dash_speed = min(current_dash_speed + (5 * g_spd), 50 * g_spd);
	            var spd = min(current_dash_speed, dist);

	            var nx = x + lengthdir_x(spd, dir);
	            var ny = y + lengthdir_y(spd, dir);

	            var hit_wall = false;
	            if (!place_meeting(nx, y, obj_wall)) x = nx; 
				
				else hit_wall = true;
	            if (!place_meeting(x, ny, obj_wall)) y = ny; 
				
				else hit_wall = true;


	            if (place_meeting(x, y, obj_player)) {
	                var kb_dir = point_direction(x, y, obj_player.x, obj_player.y);
	                var kb_dist = 12; 
	                if (!place_meeting(obj_player.x + lengthdir_x(kb_dist, kb_dir), obj_player.y, obj_wall)) {
	                    obj_player.x += lengthdir_x(kb_dist, kb_dir);
	                }
	                if (!place_meeting(obj_player.x, obj_player.y + lengthdir_y(kb_dist, kb_dir), obj_wall)) {
	                    obj_player.y += lengthdir_y(kb_dist, kb_dir);
	                }
	            }

	            if (dist < 5 || hit_wall) {
					

					
	                current_path_index++;
	                part_particles_create(global.p_sys, x, y, global.p_spark, 15);
	                if (current_path_index >= array_length(path_points)) {
	                    state = 0;
	                    timer = base_timer;
	                    gm.boss_slow = false;
	                }
	            }
	        } else {
	            state = 0;
	            timer = base_timer;
	            gm.boss_slow = false;
	        }
	        break;
	}

	var c_spd = point_distance(xprevious, yprevious, x, y);

	var tx = (state == 4) ? 1.15 + dsin(current_time * 0.8) * 0.2 : 1 + (c_spd * 0.008);
	var ty = (state == 4) ? 1.15 - dsin(current_time * 0.8) * 0.2 : 1 - (c_spd * 0.006);

	var lerp_spd = clamp(0.4 * g_spd, 0, 1);

	visual_xscale = lerp(visual_xscale, clamp(tx, 0.7, 1.5), lerp_spd);
	visual_yscale = lerp(visual_yscale, clamp(ty, 0.7, 1.5), lerp_spd);
}