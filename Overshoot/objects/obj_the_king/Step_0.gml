if (global.paused == false) {
    ...
    if (enemy_hp <= 0 && state != 9) {
        state = 9
        timer = 180
        dialogue_created = false
    }

    if (state != 2 && state != 3 && state != 8 && state != 9) {
        // UPDATED: More robust un-stick loop (increased radius to 150, added diagonal checks)
        ...
        // UPDATED: Push player without forcing them into walls
        if (place_meeting(x, y, obj_player)) {
            var push_dir = point_direction(x, y, px, py);
            var push_x = lengthdir_x(5 * g_spd, push_dir);
            var push_y = lengthdir_y(5 * g_spd, push_dir);
            
            with (obj_player) {
                if (!place_meeting(x + push_x, y, obj_wall)) x += push_x;
                if (!place_meeting(x, y + push_y, obj_wall)) y += push_y;
            }
        }
    }
    ...
    switch (state) {
        ...
        case 3:
            ...
            if (z_offset >= 0) {
                ...
                if (point_distance(x, y, px, py) < 100) {
                    obj_player.hp -= 35
                    // UPDATED: Leap knockback now respects walls
                    var k_dir = point_direction(x, y, px, py)
                    var kx = lengthdir_x(45, k_dir)
                    var ky = lengthdir_y(45, k_dir)
                    
                    with (obj_player) {
                        if (!place_meeting(x + kx, y, obj_wall)) x += kx;
                        if (!place_meeting(x, y + ky, obj_wall)) y += ky;
                    }
                }
            }
            break
        ...
        case 9:
            ...
            textbox_say(["Impossible... my reign...", "It ends here..."], c_orange, true, 30)
            
            
            global.game_ended = true
            
            visual_xscale = lerp(visual_xscale, base_scale * 1.4, 0.05 * g_spd)
            visual_yscale = lerp(visual_yscale, base_scale * 0.6, 0.05 * g_spd)
            
            if (irandom(2) == 0) {
                part_particles_create(global.p_sys, x + random_range(-40, 40), y + random_range(-40, 40), global.p_spark, 3)
            }
            
            if (timer <= 0) {
                part_particles_create(global.p_sys, x, y, global.p_spark, 200)
                instance_destroy()
            }
            break
    }
